...
class Site:
    ...
    def __init__(self, name=None, altitude=None, latitude=None, longitude=None, **kwargs):
        ...
        if "transmission" in kwargs:
            ...
            try:
                ...
                header = {}
                self.transmission = BaseUnitlessSpectrum(
                    modelclass,
                    points=wavelengths,
        ...
        if "radiance" in kwargs:
            ...
            self.radiance = read_element(
                kwargs["radiance"], element_type="radiance", flux_units=radiance_unit
            )
    ...
    def sky_spectrum_from_filter(self, filtername="V"):
        ...
        # XXX Feels like these 3 lines could be combined...
        sky_flux = np.empty(len(waveset))
        sky_flux.fill(self._photon_rate(filtername).value)
    ...
    def _photon_rate(self, filtername="V"):
        ...
        m_0 = flux_mag0_Jy.to(
            u.photon / u.cm**2 / u.s / u.angstrom, equivalencies=u.spectral_density(wavelength)
        )
    ...
    def __str__(self):
        return "{}: lon={}, lat={}, altitude={}".format(
            self.name, self.longitude, self.latitude, self.altitude
        )
...
class Telescope:
    def __init__(self, name=None, size=0, area=0, num_mirrors=2, **kwargs):
        ...
        try:
            ...
            header = {}
            mirror_se = BaseUnitlessSpectrum(
                modelclass, points=wavelengths, lookup_table=refl, keep_neg=True, meta={"header": header}
            )
    ...
    def __str__(self):
        return "{} (M1: {} diameter, {} area; {} mirrors)".format(
            self.name, self.size.to(u.m), self.area.to(self.area_unit), self.num_mirrors
        )
...
class Instrument:
    ...
    def __init__(self, name=None, inst_type="IMAGER", **kwargs):
        ...
        header = {}
        self.transmission = SpectralElement(
            Empirical1D, points=wavelengths, lookup_table=trans, keep_neg=True, meta={"header": header}
        )
        ...
        for channel in channels:
            ...
            if self.channelset[channel].ccd_pixsize != 0 and self.focal_scale != 0:
                self.channelset[channel].ccd_pixscale = self.focal_scale.to(
                    u.arcsec / u.mm
                ) * self.channelset[channel].ccd_pixsize.to(u.mm)
    ...
    def echelle_constant(self):
        if self._echelle_constant is None:
            self._echelle_constant = (
                2.0
                * self.grating_spacing
                * np.cos(np.radians(self.grating_gamma))
                * np.sin(np.radians(self.grating_blaze))
            )
    ...
    def ccd_numpixels(self):
        numpixels = [
            (int(c.ccd_xpixels / c.ccd_xbinning), int(c.ccd_ypixels / c.ccd_ybinning))
            for c in self.channelset.values()
        ]
        if len(numpixels) == 1:
    ...
    def ccd_fov(self, fov_units=u.arcsec):
        ...
        for camera in self.channelset.values():
            ...
            if (
                camera.ccd_pixsize is not None
                and self.focal_scale is not None
                and camera.ccd_xpixels is not None
                and camera.ccd_ypixels is not None
            ):
...
class Camera:
    def __init__(self, name=None, camera_type="CCD", **kwargs):
        ...
        if not isinstance(ccd_qe, (u.Quantity, numbers.Number)):
            ...
            self.ccd_qe = BaseUnitlessSpectrum(
                Empirical1D,
                points=wavelengths,
                lookup_table=throughput,
                keep_neg=False,
        ...
        if self.ccd_xbinning != self.ccd_ybinning:
            raise ETCError(
                f"Non-equal binnings (X={self.ccd_xbinning:}, Y={self.ccd_ybinning:}) are not supported at this time"
            )
