...
class TestETC:
    ...
    def test_initialize_from_dict(self):
        config_dict = {
            "site": {"name": "La Palma, Spain", "transmission": 0.8709635899560807},
            "telescope": {
                "name": "WHT 4.2m",
                "size": 4.2,
                "area": 12.47,
        ...
        assert "<ETC [Site(La Palma, Spain)]->[Telescope(WHT 4.2m)]->[Instrument(WHT PFIP)]>" == repr(
            test_etc
        )

    def test_initialize_config_file(self):
        test_config_file = files("tests.etc.data").joinpath("test1.toml")
    ...
    def test_throughput_for_filter_noatmos_singlechannel(self):
        ...
        config_dict = {
            "site": {"name": "Tenerife, Spain", "transmission": 0.9},
            "telescope": {
                "name": "LCO 1.0m",
                "size": 1.0,
                "area": 0.63,
    ...
    def test_channel_for_filter_dualchannel(self):
        config_dict = {
            "instrument": {
                ...
                "ccd_pixsize": 15.0,
                "channels": {
                    "blue_channel": {"filterlist": ["gp"], "ccd_qe": 0.85},
...
class TestPhotonsFromSource:
    ...
    def setUp(self):
        self.test_config_file = os.path.abspath(
            os.path.join(__package__, "etc", "tests", "data", "test1.toml")
        )
        self.WHT_config = {
            "site": {"name": "La Palma, Spain", "transmission": 0.8709635899560807},
            "telescope": {
...
class TestCCDSNR:
    @pytest.fixture(autouse=True)
    def setUp(self):
        self.test_config_file = files("tests.etc.data").joinpath("test1.toml")
        self.WHT_config = {
            "site": {"name": "La Palma, Spain", "transmission": 0.8709635899560807},
            "telescope": {
                "name": "WHT 4.2m",
    ...
    def test_WHT_1s_B15(self):
        ...
        airmass = 1.5
        self.WHT_config["site"]["transmission"] = 10.0 ** (-0.4 * extin * airmass)
        self.WHT_config["instrument"]["filterlist"] = [
            "WHT::B",
        ]
